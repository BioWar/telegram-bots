...
def getLinks(url):
    ...
    for i in soup.find_all('a', attrs={'href': re.compile("^https://")}):
        link = i.get('href')
        if 'https://medium.com/m/signin?operation=register' not in link and \
           'https://towardsdatascience.com/?source' not in link and \
           'https://medium.com/m/signin?' not in link and \
           'https://medium.com/membership?s' not in link:
            links.add(link)
...
if __name__=='__main__':
    ...
    while True:
        ...
        try:
            ...
            bot.send_message(CHANNEL_NAME, "Looks like we are out of articles for today. Come here tomorrow pal :)")    
            append_table()
            sleep(30)
